...
impl Sandbox for RecipeManagerGUI {
    ...
    fn update(&mut self, message: Message) {
        match message {
            ...
            Message::LoadRecipes => {
                match self.recipe_manager.load_from_file("
recipes
.json
") {
                    Ok(_) => {
                    ...
                    Err(e) => {
                        self.error_message = Some(format!("Failed to load recipes: {}", e));
                    }
                }
            }
        }
    ...
    fn view(&self) -> Element<Message> {
        ...
        let recipes: Element<_> = self.recipe_manager
...
Recipes
")).on_press(Message::SaveRecipes))

            .push(Button::new(Text::new("Load
 
Recipes
")).on_press(Message::LoadRecipes));
